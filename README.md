...
3. **Back me up** — **Tap** for manual tracking: the app does **not** start local playback. Instead it starts a user-clocked track timer, listens to the amp, and tries to measure the trail gap from local matches near the expected file time. **Hold** the button for auto lock-on: backup stays silent, trailing the amp by the trail delay, until two matching clips confirm lock.
